            ...
            for buf in range(0, 8):
                    ...
                    if (v_samp <= 0.) and (v_samp_p1 > 0.):
                        ...
                        if last_rising > -1:
                            if last_rising > k+1: # or last_rising == 0:
                                period_cells[l][k+1].append( interp_time - time_last_rising + prim_cells * nominal_dt)
                            else:
                    ...
                    if (v_samp >= 0.) and (v_samp_p1 < 0.):
                        ...
                        if last_falling > -1:
                            if last_falling > k+1: # or last_falling == 0:
                                period_cells[l][k+1].append( interp_time - time_last_falling + prim_cells * nominal_dt)
                            #    if k+1 == 12:
                ...
                if  (buf < 7):
                    ...
                    if  (v_samp <= 0.) and (v_samp_p1 > 0.):
                        ...
                        if last_rising > -1:
                            period_cells[l][0].append( interp_time - time_last_rising + prim_cells * nominal_dt )
                    ...
                    if  (v_samp >= 0.) and (v_samp_p1 < 0.):
                        ...
                        if last_falling > -1:
                            period_cells[l][0].append( interp_time - time_last_falling + prim_cells * nominal_dt )
...
if __name__=='__main__':
    ...
    d = np.loadtxt('235MHzTestTrims.dat')
    ...
    plt.figure()
    plt.plot(crossings[lab]* 1.e6/ freq, '-', drawstyle='steps')
    plt.xlabel('primary cell number')
    plt.ylabel('dt')

    #plt.figure()
    #plt.hist(crossings[lab] * 1.e6/ freq, bins=range(250, 391, 8))
    #plt.xlabel('dT [ps]')

    print np.mean(crossings[lab][:] * 1.e6/ freq)
    print np.std(crossings[lab][:-1] * 1.e6/ freq)

    integral_nonlinearity = np.zeros(prim_cells, dtype=float)
    t_calc = np.zeros(prim_cells+1, dtype=float)

    period_mean=np.zeros(prim_cells, dtype=float)
    for i in range(prim_cells):
        period_mean[i] = np.mean(period[lab][i])
        #if i < prim_cells-1:
        t_calc[i+1] = np.cumsum(crossings[lab][:i+1])[-1]* 1.e3/freq
        #print i, t_calc[i], t_nom[i]
    #print '128', t_calc[128], t_nom[128] 

    integral_nonlinearity = t_calc-t_nom

    plt.figure()
    plt.plot(integral_nonlinearity)

    plt.figure()
    bins= np.arange(3.2, 5.4, 0.01)
    #plt.hist(period[lab][0], alpha=0.5)
    plt.hist(period[lab][1], alpha=0.5, bins=bins)
    plt.hist(period[lab][10], alpha=0.5, bins=bins)

    plt.hist(period[lab][30], alpha=0.5, bins=bins)
    plt.hist(period[lab][50], alpha=0.5, bins=bins)
    plt.hist(period[lab][100], alpha=0.5, bins=bins)
    #plt.hist(period[lab][120], alpha=0.5, bins=np.arange(4.0, 5.4, 0.01))

    plt.figure()
    plt.plot(period_mean, 'o-')
    plt.plot([0,128], [1.e3/freq, 1.e3/freq], '--')
